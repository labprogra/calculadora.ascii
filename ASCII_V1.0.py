...
def validation(fila):
	...
	if (fila == True):
		print ERROR
		print 'El largo de alguna(s) fila(s) no corresponde(n) al requerido, modifique el archivo'
		correct = False
	else:
	...
	if error:
		print ERROR
		print "algun(os) caracter(es) no corresponde(n) a lo solicitado, modifique el archivo"
		correct = False
	return correct
...
topFrame = Frame(root)
topFrame.pack(pady = 50, padx = 50)

text = Label(topFrame, text="Ingresar el nombre del archivo:")
text.grid(columnspan=2,row=0)

message = Label(topFrame, text="", font=calibri)
message.grid(columnspan=2, row=4)

Operation = Label(topFrame, text="")
Operation.grid(columnspan=3, row=3)

Matriz = Label(topFrame, text="", font=font)
Matriz.grid(columnspan=7, row=4)
Result = Label(topFrame, text="")
Result.grid(columnspan=3, row=5)

OUTPUT = Label(topFrame, text="")
OUTPUT.grid(columnspan=3, row=6)

box = Entry(topFrame, textvariable="")
box.grid(row=1)

submitFile = Button(topFrame, text="Buscar", fg="black", command=main)
submitFile.grid(column=1,row=1)

status = Label(root, text="Version 1.0 - Calculadora ASCII", bd=1, relief=SUNKEN, anchor=W)
